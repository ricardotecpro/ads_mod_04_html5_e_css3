...
Em vez de usar pixels (`px`) para tamanhos de fonte e espaçamentos, a prática moderna prefere unidades relativas como `rem`.
  ...
  * **`rem` (root em)**: Unidade relativa ao tamanho da fonte da tag `<html>`. Se o usuário aumentar o tamanho da fonte padrão do navegador por questões de acessibilidade, toda a sua interface se ajustará proporcionalmente.
...
BEM (Block, Element, Modifier) é uma convenção de nomenclatura que torna seu CSS mais legível, reutilizável e escalável.
  ...
  * **Block**: Um componente autônomo (`.card`, `.nav`).
  * **Element**: Uma parte de um bloco (`.card__title`, `.nav__link`).
  * **Modifier**: Uma variação de um bloco ou elemento (`.card--dark`, `.nav__link--active`).
